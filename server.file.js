...
const app = express();
const PORT = 5000;
const JWT_SECRET = 'HomWisor-secret-key-2026';
...
function loadData() {
  ...
  try {
    ...
    // ensure properties have new fields
    if (parsed.properties) {
      ...
      defaultData.properties.forEach(d => {
        if (!parsed.properties.find(p => p.id === d.id)) parsed.properties.push(d);
      });
    }
...
app.get('/api/snaps', (req, res) => res.json(db.snaps));
app.get('/api/settings', (req, res) => res.json(db.settings));
app.get('/api/builders', (req, res) => res.json(db.builders||[]));
...
app.get('/api/snaps/:id', (req, res) => {
  const snap = db.snaps.find(s => s.id === req.params.id);
...
app.put('/api/properties/:id', authMiddleware, (req, res) => {
  ...
  db.properties[idx] = { ...db.properties[idx], ...req.body, id: req.params.id };
  saveData(db);
...
app.post('/api/snaps', authMiddleware, (req, res) => {
  const newSnap = { id: 'snap' + Date.now(), ...req.body };
  db.snaps.push(newSnap);
  saveData(db);
  res.json(newSnap);
});
app.put('/api/snaps/:id', authMiddleware, (req, res) => {
  const idx = db.snaps.findIndex(s => s.id === req.params.id);
  if (idx === -1) return res.status(404).json({ error: 'Not found' });
  db.snaps[idx] = { ...db.snaps[idx], ...req.body, id: req.params.id };
  saveData(db);
  res.json(db.snaps[idx]);
});
app.delete('/api/snaps/:id', authMiddleware, (req, res) => {
  const before = db.snaps.length;
  db.snaps = db.snaps.filter(s => s.id !== req.params.id);
  if (db.snaps.length === before) return res.status(404).json({ error: 'Not found' });
  saveData(db);
  res.json({ success: true });
});

app.post('/api/banners/:type', authMiddleware, (req, res) => {
  const type = req.params.type;
  if (!['hero','small'].includes(type)) return res.status(400).json({error:'invalid type'});
  const newBanner = { id: type[0] + Date.now(), ...req.body };
  db.banners[type].push(newBanner);
  saveData(db);
  res.json(newBanner);
});
app.put('/api/banners/:type/:id', authMiddleware, (req, res) => {
  const { type, id } = req.params;
  const idx = db.banners[type]?.findIndex(b=>b.id===id);
  if(idx===-1 || idx===undefined) return res.status(404).json({error:'Not found'});
  db.banners[type][idx] = {...db.banners[type][idx], ...req.body, id};
  saveData(db);
  res.json(db.banners[type][idx]);
});
app.delete('/api/banners/:type/:id', authMiddleware, (req, res) => {
  const { type, id } = req.params;
  const before = db.banners[type].length;
  db.banners[type] = db.banners[type].filter(b=>b.id!==id);
  if(before===db.banners[type].length) return res.status(404).json({error:'Not found'});
  saveData(db);
  res.json({success:true});
});
...
app.put('/api/locations/:id', authMiddleware, (req,res)=>{
  ...
  if(idx===-1) return res.status(404).json({error:'Not found'});
  db.locations[idx]={...db.locations[idx],...req.body, id:req.params.id};
  saveData(db);
  res.json(db.locations[idx]);
});
app.delete('/api/locations/:id', authMiddleware, (req,res)=>{
...
app.post('/api/offers', authMiddleware, (req,res)=>{
  const newOffer={ id:'o'+Date.now(), ...req.body };
...
app.put('/api/offers/:id', authMiddleware, (req,res)=>{
  ...
  if(idx===-1) return res.status(404).json({error:'Not found'});
  db.offers[idx]={...db.offers[idx],...req.body, id:req.params.id};
  saveData(db);
...
app.post('/api/builders', authMiddleware, (req,res)=>{ const b={id:'b'+Date.now(),...req.body}; db.builders.push(b); saveData(db); res.json(b); });
app.delete('/api/builders/:id', authMiddleware, (req,res)=>{ db.builders=db.builders.filter(b=>b.id!==req.params.id); saveData(db); res.json({success:true}); });
app.post('/api/testimonials', authMiddleware, (req,res)=>{ const t={id:'t'+Date.now(),...req.body}; db.testimonials.push(t); saveData(db); res.json(t); });
app.delete('/api/testimonials/:id', authMiddleware, (req,res)=>{ db.testimonials=db.testimonials.filter(t=>t.id!==req.params.id); saveData(db); res.json({success:true}); });
app.get('/api/enquiries', authMiddleware, (req,res)=> res.json(db.enquiries));
app.post('/api/enquiries', (req,res)=>{
  const newEnq = { id:'e'+Date.now(), date:new Date().toISOString().slice(0,10), ...req.body };
  db.enquiries.unshift(newEnq);
  saveData(db);
  res.json(newEnq);
});
app.delete('/api/enquiries/:id', authMiddleware, (req,res)=>{
...
app.get('/api/admin/stats', authMiddleware, (req,res)=>{
  res.json({
    ...
    totalSnaps: db.snaps.length,
    totalBuilders: (db.builders||[]).length,
    totalTestimonials: (db.testimonials||[]).length,
    recommended: db.properties.filter(p=>p.category==='recommended').length,
    trending: db.properties.filter(p=>p.category==='trending').length
  });
});
